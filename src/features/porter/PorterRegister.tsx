...
import PorterEntryForm from '@features/porter/PorterEntryForm';
...
const PorterRegister: React.FC<Props> = ({ trips, onNewTrip, onUpdateTrip, onDeleteTrip, onLogout }) => {
    ...
    const [showDeleteModal, setShowDeleteModal] = useState<PorterTrip | null>(null);
    ...
    const handleStatusUpdate = (trip: PorterTrip, status: 'PICKUP' | 'TRANSIT' | 'DELIVERED') => {
        let newDeliveryStatus: DeliveryStatus = 'ACCEPTED';
        let remarks = '';
        if (status === 'PICKUP') {
            newDeliveryStatus = 'PICKUP_STARTED';
            remarks = 'Pickup Started';
        } else if (status === 'TRANSIT') {
            newDeliveryStatus = 'ON_THE_WAY';
            remarks = 'In Transit';
        ...
        const updatedTimeline = [
            ...(trip.timeline || []),
            { status: newDeliveryStatus, timestamp: new Date().toISOString(), remarks }
        ];

        const updatedTrip: PorterTrip = {
            ...trip,
        ...
        sendCustomerUpdate(trip, status);
        logAction('UPDATE', updatedTrip.id, `Status updated to ${newDeliveryStatus} for ${updatedTrip.customerName}`, 'GAURAV PANCHAL');
    };

    const executeDelete = () => {
        if (deletePasscode === '0001') {
            if (showDeleteModal) {
                onDeleteTrip(showDeleteModal.id);
                logAction('DELETE', showDeleteModal.id, `Permanently deleted porter mission for ${showDeleteModal.customerName}`, 'GAURAV PANCHAL');
                ...
                <div className="flex items-center gap-4 md:gap-6 w-full md:w-auto">
                    <div className="flex flex-col">
                        <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter flex items-center gap-3">
                            PORTER COMMAND
                            <span className="text-[10px] font-black bg-emerald-50 text-emerald-600 px-2 py-1 rounded-md border border-emerald-100 flex items-center gap-1 shadow-sm">
                                <Shield className="w-3 h-3" /> PROTECTED
                            </span>
                        </h1>
                        <div className="flex items-center gap-2 mt-1">
                            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]"></div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">KM-Wise Logistics & Delivery Ledger</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-between md:justify-end gap-4 w-full md:w-auto mt-2 md:mt-0">
                    <div className="flex bg-slate-100 p-1.5 rounded-2xl gap-1">
                        <button 
                            ...
                            className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all ${view === 'DASHBOARD' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                            Dashboard
                        </button>
                        <button 
                            onClick={() => setView('LOGBOOK')}
                            className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all ${view === 'LOGBOOK' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                            Logbook
                        </button>
                        <button 
                    ...
                    <button 
                        ...
                        className="bg-white border border-slate-200 text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 font-black px-5 h-10 rounded-2xl flex items-center gap-2 text-[10px] uppercase tracking-widest transition-all shadow-sm"
                    >
                        <Package className={`w-4 h-4 ${isMigrating ? 'animate-spin' : ''}`} /> 
                        {isMigrating ? 'SYNCING...' : 'CLOUD SYNC'}
                    </button>
                    <button 
                        onClick={() => setIsFormOpen(true)}
                        className="bg-indigo-600 hover:bg-indigo-700 text-white font-black px-6 h-10 rounded-2xl flex items-center gap-2 text-[10px] uppercase tracking-widest transition-all shadow-lg shadow-indigo-500/25"
                    >
                    ...
                    {onLogout && (
                        <button 
                            onClick={onLogout}
                            className="bg-white border border-slate-200 text-slate-400 hover:text-rose-600 hover:bg-rose-50 hover:border-rose-200 font-black w-10 h-10 rounded-2xl flex items-center justify-center transition-all shadow-sm"
                        >
                                    ...
                                    <div className="space-y-10 flex-1">
                                        <div>
                                            <div className="flex justify-between items-end mb-4">
                                                ...
                                                <p className="text-3xl font-black text-emerald-400">{trips.length ? Math.round((stats.deliveredCount/trips.length)*100) : 0}%</p>
                                            </div>
                                            <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                                                <div className="h-full bg-emerald-500 rounded-full shadow-[0_0_10px_rgba(16,185,129,0.5)]" style={{ width: `${trips.length ? (stats.deliveredCount/trips.length)*100 : 0}%` }}></div>
                                            </div>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-6">Payment Distribution</p>
                                            <div className="flex gap-2 h-3 bg-slate-800 rounded-full overflow-hidden">
                                    ...
                                    <div className="mt-10 pt-10 border-t border-slate-800">
                                        <div className="flex flex-col gap-4">
                                            <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-slate-500">
                                                ...
                                                <span className={PorterProtectionAgent.verifyIntegrity(trips) ? 'text-emerald-400' : 'text-rose-400'}>
                                                    {PorterProtectionAgent.verifyIntegrity(trips) ? 'VERIFIED' : 'FAILED'}
                                                </span>
                                            </div>
                                            <button className="w-full bg-slate-800/50 hover:bg-slate-800 text-white font-black py-4 rounded-2xl transition-all flex items-center justify-center gap-3 border border-slate-700 text-[10px] uppercase tracking-widest">
                                                <FileText className="w-4 h-4 text-indigo-400" /> Export Forensic Audit
                                            </button>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="hidden md:block bg-white rounded-[3rem] border border-slate-100 shadow-xl overflow-hidden">
                            <div className="overflow-x-auto no-scrollbar">
                                <table className="w-full text-left">
                                    <thead>
                                        <tr className="bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                                            <th className="py-8 px-10">Mission / Date</th>
                                            <th className="py-8 px-6">Porter / Route</th>
                                            <th className="py-8 px-6">Distance / KM</th>
                                            <th className="py-8 px-6">Valuation</th>
                                            {showAdvancedData && <th className="py-8 px-6">Lifecycle</th>}
                                            ...
                                            <tr key={trip.id} className="group hover:bg-slate-50/50 transition-all border-b border-slate-50 last:border-0">
                                                <td className="py-8 px-10">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <p className="font-black text-slate-900 text-sm tracking-tight">{trip.id}</p>
                                                            <p className="text-[10px] font-black text-slate-400 uppercase">{trip.date}</p>
                                                            {showAdvancedData && <p className="text-[9px] font-bold text-slate-400 mt-1 uppercase">Customer: {trip.customerName}</p>}
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="py-8 px-6">
                                                    <div className="space-y-1">
                                                        ...
                                                        {showAdvancedData && <p className="text-[9px] font-black text-slate-400 uppercase mt-1">Vehicle: {trip.vehicleNumber}</p>}
                                                    </div>
                                                </td>
                                                <td className="py-8 px-6">
                                                    <div className="flex flex-col">
                                                ...
                                                <td className="py-8 px-6">
                                                    <div className="space-y-1">
                                                        ...
                                                        {showAdvancedData && <p className="font-black text-slate-400 text-[10px] uppercase">Gross: ₹{trip.grossAmount.toLocaleString()}</p>}
                                                        <button 
                                                ...
                                                <td className="py-8 px-10 print:hidden">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <div className="flex items-center bg-slate-50 p-1 rounded-xl gap-1">
                                                            <button onClick={() => handleStatusUpdate(trip, 'PICKUP')} title="Pickup Alert" className="p-2 text-slate-400 hover:text-blue-500 transition-colors"><Map className="w-4 h-4" /></button>
                                                            <button onClick={() => handleStatusUpdate(trip, 'TRANSIT')} title="Transit Alert" className="p-2 text-slate-400 hover:text-amber-500 transition-colors"><Navigation2 className="w-4 h-4" /></button>
                                                            <button onClick={() => handleStatusUpdate(trip, 'DELIVERED')} title="Delivery Alert" className="p-2 text-slate-400 hover:text-emerald-500 transition-colors"><CheckCircle className="w-4 h-4" /></button>
                                                        </div>
                                                        ...
                                                        <button 
                                                            onClick={() => setShowDeleteModal(trip)}
                    ...
                    <div className="bg-white rounded-[3rem] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                        <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-red-50/30">
                            <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter flex items-center gap-3">
                                <AlertCircle className="w-6 h-6 text-red-500" /> Authorized Deletion
                            </h3>
                        ...
                        <div className="p-10 space-y-6 text-center">
                            <p className="text-sm text-slate-500 font-medium">You are about to permanently remove porter entry <span className="font-black text-slate-900">{showDeleteModal.id}</span> from the registry.</p>
                            
                            <div className="space-y-3">
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Enter Deletion Passcode</label>
                                <input 
                                    type="password" 
