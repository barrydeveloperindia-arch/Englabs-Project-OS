...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_number(self, page_count):
        ...
        self.drawString(35, 20, "Confidential - July 2026 Petty Cash Ledger")
        self.restoreState()
...
def main():
    ...
    output_pdf = os.path.join(output_dir, 'July_2026_Detailed_Petty_Cash_Report.pdf')
    ...
    # Cols 10: Maintenance, 11: Emergency, 12: Zepto/Blinkit, 13: Sky5 Hotel, 14: Mr. Behl, 15: Project Dr.
    categories_cols = {
        'J': 'Maintenance',
        'K': 'Emergency',
        'L': 'Zepto / Blinkit',
    ...
    # Rows 6 to 17 are the active transactions in July
    for r in range(6, 18):
        ...
        proj_id = sheet.cell(row=r, column=16).value # Project ID is in Col P (Col 16)
        proj_dr = sheet.cell(row=r, column=15).value # Project Dr. is in Col O (Col 15)
        ...
        # Staging: If project ID is empty but description contains C5254 (Row 17)
        if not pid_str and 'C5254' in desc_str:
            pid_str = 'C5254'
    ...
    meta_data = [
        [Paragraph("Reporting Period:", cell_bold_style), Paragraph("July 2026 (Current Status)", body_style)],
    ...
    This report contains the live financial statement and transaction audit log of Englabs Projects' petty cash ledger for July 2026. This report delivers strategic insights by cross-referencing actual project expenditures with pre-defined courier budgets (Vendor to Englabs and Englabs to Client). Underperforming project nodes and cost over-runs are highlighted in the consolidated audit table to facilitate management cost controls.
    """
    ...
    outflow_data = [
        ...
        [Paragraph("Current Net Balance", cell_bold_style), Paragraph(f"₹ {total_cr - total_dr:,.2f}", cell_bold_style)]
    ]
    ...
    # II. July Project Courier Budget & Expenditure Audit
    story.append(Paragraph("II. July Project Courier Budget & Expenditure Audit", section_title_style))
    ...
    print("SUCCESS: Premium July report PDF generated.")
