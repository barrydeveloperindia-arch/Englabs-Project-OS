...
import { FoodOrder, PLATFORMS, generateFoodId, FoodPlatform } from '@shared/services/food_system';
import FoodOrderForm from '@modules/inventory/food/FoodOrderForm';
import FoodReceiptSlip from '@modules/inventory/food/FoodReceiptSlip';
                            ...
                            <div className="p-8 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/60 backdrop-blur-md">
                                <div className="flex items-center gap-4 flex-1">
                                    ...
                                    <button 
                                        ...
                                        className={`flex items-center gap-2 px-4 py-2.5 border rounded-xl text-[10px] font-black transition-all ${
                                            showAdvancedLiveRegister 
                                                ? 'bg-emerald-50 border-emerald-200 text-emerald-600 dark:bg-emerald-500/10 dark:border-emerald-500/20 dark:text-emerald-400' 
                                                : 'bg-white dark:bg-slate-900/50 border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:text-white'
                                        }`}
                                    >
                                        <div className={`w-2 h-2 rounded-full ${showAdvancedLiveRegister ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'}`} />
                                        ADVANCED OPTIONS
                                    </button>
                                </div>
                                            ...
                                            <tr key={order.entryId} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors group">
                                                ...
                                                <td className="px-8 py-6">
                                                    <div>
                                                        ...
                                                        {showAdvancedLiveRegister && <p className="text-[10px] font-bold text-slate-400 uppercase">{order.department}</p>}
                                                    </div>
                                                </td>
                                                <td className="px-8 py-6">
                                                    <div>
