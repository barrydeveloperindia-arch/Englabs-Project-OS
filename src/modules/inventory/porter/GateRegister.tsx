...
import GateEntryForm from '@modules/inventory/porter/GateEntryForm';
import GatePassSlip from '@modules/inventory/porter/GatePassSlip';
import GateInvoiceSlip from '@modules/inventory/porter/GateInvoiceSlip';
import { GateEntry, generateId, generateGatePassId, UNITS, DELIVERY_TYPES } from '@shared/services/gate_system';
...
const GateRegister: React.FC<Props> = ({ entries, onNewEntry, onUpdateEntry, onDeleteEntry, onLog, onFullSync }) => {
    ...
    return (
        <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-[#F8FAFC]">
            <header className="h-auto md:h-24 bg-white border-b border-slate-100 flex flex-col xl:flex-row items-start xl:items-center justify-between px-4 md:px-10 py-4 md:py-0 shrink-0 shadow-sm z-20 gap-4 xl:gap-0">
                <div className="flex flex-col xl:flex-row items-start xl:items-center gap-4 xl:gap-6 w-full xl:w-auto">
                    <div className="flex flex-col">
                        <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter">GATE COMMAND</h1>
                        <div className="flex items-center gap-2 mt-1">
                            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]"></div>
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Logistics Registry</span>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center bg-slate-100 p-1.5 rounded-2xl gap-1">
                        ...
                        <button 
                            className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all ${view === 'INWARD_LIST' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`} 
                            onClick={() => setView('INWARD_LIST')}
                        >
                            Inward Registry <span className="ml-1 opacity-60">({stats.totalInwardToday})</span>
                        </button>
                        <button 
                            className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all ${view === 'OUTWARD_LIST' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`} 
                            onClick={() => setView('OUTWARD_LIST')}
                        >
                            Outward Registry <span className="ml-1 opacity-60">({stats.totalOutwardToday})</span>
                        </button>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2 md:gap-4 w-full md:w-auto justify-between md:justify-end mt-2 md:mt-0">
                    <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 border border-slate-100 rounded-2xl">
                        <div className={`w-2 h-2 rounded-full ${dbStatus === 'SECURE' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)] animate-pulse' : 'bg-amber-500'}`}></div>
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Database: {dbStatus}</span>
                    </div>
                    <button 
                        onClick={syncDatabase}
                        disabled={isSyncing}
                        data-testid="btn-sync-cloud"
                        className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all ${isSyncing ? 'bg-slate-50 text-slate-400' : 'bg-white text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 border border-slate-200 hover:border-indigo-200 shadow-sm'}`}
                    ...
                    <button 
                        ...
                        className="h-10 px-4 bg-white border border-slate-200 text-slate-600 rounded-2xl hover:bg-slate-50 hover:text-slate-800 transition-all shadow-sm flex items-center gap-2"
                        title="Master Excel Export"
                    >
                        <Download className="w-4 h-4" />
                        <span className="text-[10px] font-black uppercase tracking-widest hidden sm:inline-block">Master Export</span>
                    </button>
                    <button 
                        onClick={() => setView('NEW_ENTRY')}
                        className="h-10 px-6 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl text-xs font-black uppercase tracking-wider transition-all shadow-lg shadow-indigo-500/25 flex items-center gap-2"
                    >
...
const StatCard = ({ icon, label, value, subValue, color, onClick }: any) => {
    const colors: any = {
        emerald: { text: "text-emerald-600", bg: "bg-emerald-50", hover: "group-hover:text-emerald-500", large: "text-emerald-500" },
        blue: { text: "text-indigo-600", bg: "bg-indigo-50", hover: "group-hover:text-indigo-500", large: "text-indigo-500" },
        purple: { text: "text-purple-600", bg: "bg-purple-50", hover: "group-hover:text-purple-500", large: "text-purple-500" },
        amber: { text: "text-amber-600", bg: "bg-amber-50", hover: "group-hover:text-amber-500", large: "text-amber-500" }
    };
    ...
    return (
        <div 
            onClick={onClick}
            className={`bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-[0_10px_40px_rgba(0,0,0,0.01)] hover:shadow-xl transition-all group relative overflow-hidden ${onClick ? 'cursor-pointer hover:-translate-y-1' : ''}`}
        >
            <div className={`absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity ${c.large}`}>
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1 relative z-10">{label}</p>
            <p className="text-3xl font-black text-slate-900 tracking-tighter relative z-10">{value}</p>
            {subValue && (
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-2 relative z-10">{subValue}</p>
            )}
        </div>
    );
};
