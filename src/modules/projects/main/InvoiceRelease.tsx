...
export const InvoiceRelease: React.FC<InvoiceReleaseProps> = ({ projects }) => {
    ...
    const shareInvoiceOnWhatsApp = (project: ProjectData) => {
        ...
        const text = encodeURIComponent(
            `*Project Invoice Release Update*\n` +
            `*Project ID:* ${project.projectId}\n` +
            `*Client:* ${project.client}\n` +
            `*Invoice Number:* ${inv.invoiceNumber || 'N/A'}\n` +
            `*Base Value:* ₹${base.toLocaleString('en-IN')}\n` +
            `*GST Rate:* ${inv.gstRate || 18}%\n` +
            `*GST Value:* ₹${gst.toLocaleString('en-IN')}\n` +
    ...
    const shareInvoiceSummaryOnWhatsApp = () => {
        const text = encodeURIComponent(
            `*ENGLABS PORTFOLIO INVOICE RELEASE SUMMARY*\n` +
            `--------------------------------\n` +
            `*Total Invoices:* ${stats.totalInvoices}\n` +
            `*Invoice Value (Base):* ₹${stats.totalBaseValue.toLocaleString('en-IN')}\n` +
            `*Total GST (18%):* ₹${stats.totalGSTValue.toLocaleString('en-IN')}\n` +
            `*Gross Billing Ledger:* ₹${stats.totalInvoiceAmount.toLocaleString('en-IN')}\n` +
            `*Collections Status:* ${stats.paidCount} of ${stats.totalInvoices} Paid (Pending: ${stats.pendingCount} | Overdue: ${stats.overdueCount})\n` +
            `--------------------------------\n` +
                ...
                <div className="bg-white p-6 md:p-8 rounded-[1.75rem] border border-slate-100 shadow-sm">
                    <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4 mb-6">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            <button 
                                onClick={shareInvoiceSummaryOnWhatsApp}
                                ...
                                title="Share Portfolio Invoice Summary on WhatsApp"
                            >
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="py-4 text-right" onClick={(e) => e.stopPropagation()}>
                                                <button 
                                                    onClick={() => shareInvoiceOnWhatsApp(project)}
