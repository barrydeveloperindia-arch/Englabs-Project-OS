...
import { FinanceTransactionForm } from '@features/erp/modules/finance/FinanceTransactionForm';
import { FileText, Landmark } from 'lucide-react';

export const AccountsFinanceModule: React.FC = () => {
    const { selectedProjectId } = useERPContext();
    const [activeTab, setActiveTab] = useState<'Live Actions'|'Accounts Master'|'Ledger'>('Live Actions');
    const [liveLedger, setLiveLedger] = useState<any[]>([]);

    useEffect(() => {
        const q = query(collection(db, 'finance_ledger'), orderBy('createdAt', 'desc'), limit(50));
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-black text-slate-800">Accounts & Finance</h1>
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl w-max mb-6">
                ...
                <button 
                    onClick={() => setActiveTab('Accounts Master')}
                    className={`px-6 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'Accounts Master' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
                    Accounts Master
                </button>
                <button 
            ...
            {activeTab === 'Live Actions' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-2xl p-12 text-center flex flex-col items-center justify-center">
                        ...
                        <h3 className="text-lg font-black text-blue-900">Live Finance Mode</h3>
                        <p className="text-sm text-blue-700 mt-2 max-w-md font-medium">
                            {selectedProjectId 
                                ? `Project locked: \${selectedProjectId}. Expense & Revenue logic will instantly update the Master Costing Engine.` 
                                : 'Select a Project ID in the global header to enable financial posting.'}
                        </p>
                    </div>
                </div>
            )}
            ...
            {activeTab === 'Ledger' && (
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="p-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
                        <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">Finance Ledger</h3>
                    </div>
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-slate-50 border-b border-slate-200">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-[10px] font-black text-slate-500 uppercase tracking-widest text-right">GST</th>
                                ...
                                <tr key={txn.id} className="border-b border-slate-100 hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center gap-1 px-2 py-1 rounded text-[10px] font-black uppercase tracking-wider \${
                                            txn.type === 'FINANCE_EXPENSE' ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700'
                                        }`}>
                                            {txn.type.replace('_', ' ')}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 text-sm font-bold text-slate-700">{txn.category}</td>
                                    <td className="px-6 py-4 text-xs font-bold text-slate-600">{txn.projectId || '-'}</td>
                                    <td className="px-6 py-4 text-xs font-medium text-slate-500 text-right">{txn.gstAmount || 0}</td>
                                    <td className={`px-6 py-4 text-sm font-black text-right \${txn.amount > 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
