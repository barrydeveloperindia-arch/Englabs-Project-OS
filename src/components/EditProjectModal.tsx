...
import { X, Save, DollarSign, Calendar, Edit2, Plus } from 'lucide-react';
    ...
    useEffect(() => {
        if (project) {
            setFormData({
                ...
                value: String(project.planning?.value || 0),
                budget: String(project.planning?.budget || 0)
            });
        }
    }, [project, isOpen]);
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <div className="relative w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
                <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <div>
                        <h2 className="text-2xl font-black text-slate-900 tracking-tight">Edit Mission Details</h2>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Project ID: {project.projectId}</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    ...
                    <button 
                        type="submit"
                        className="w-full bg-[#0e4368] text-white font-black py-5 rounded-2xl hover:bg-slate-800 hover:shadow-[0_10px_30px_rgba(14,67,104,0.3)] transition-all active:scale-[0.98] flex items-center justify-center gap-3 mt-4"
                    >
