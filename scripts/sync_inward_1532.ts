...
async function run() {
    try {
        console.log("Loading invoice gate entry IN-1532...");
        const entry = forensicRegistry.find((e: any) => e.id === 'IN-1532');
        if (!entry) {
            console.error("Entry IN-1532 not found in local registry!");
            process.exit(1);
        }
