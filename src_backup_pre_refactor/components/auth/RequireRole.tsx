...
export type UserRole = 'ADMIN' | 'STAFF' | 'STORE_MANAGER' | 'HR_ADMIN' | 'FLOOR_MANAGER';
...
export const RequireRole: React.FC<RequireRoleProps> = ({ allowedRoles, children }) => {
    ...
    // ADMIN is the superuser who can access anything, otherwise check if userRole is in allowedRoles
    const hasAccess = userRole === 'ADMIN' || (userRole && allowedRoles.includes(userRole));
