...
import { PorterTrip, calculatePorterAmount, PorterPaymentStatus } from '@domain/porter_system';
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        <div className="flex justify-center border-t border-slate-100 pt-6">
                            <button
                                ...
                                className="text-xs font-black text-slate-400 hover:text-emerald-500 transition-colors uppercase tracking-widest bg-slate-50 px-4 py-2 rounded-xl"
                            >
                                {showAdvancedOptions ? '- Hide Advanced Options' : '+ Show Advanced Options (Customer, Expenses, Advance)'}
                            </button>
                        </div>
                        ...
                        {showAdvancedOptions && (
                            ...
                            <div className="grid grid-cols-2 gap-6 mb-6">
                                <div className="space-y-2">
                                    ...
                                    <input type="text" required={showAdvancedOptions} className="w-full bg-white border border-slate-200 rounded-xl py-3 px-4 text-xs font-bold" placeholder="Full Name" value={formData.customerName || ''} onChange={e => setFormData({ ...formData, customerName: e.target.value })} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Mobile Number</label>
                                    <input type="tel" required={showAdvancedOptions} className="w-full bg-white border border-slate-200 rounded-xl py-3 px-4 text-xs font-bold" placeholder="Contact No." value={formData.customerMobile || ''} onChange={e => setFormData({ ...formData, customerMobile: e.target.value })} />
                                </div>
                            </div>
                            <div className="space-y-2">
                                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Delivery Address</label>
                                <textarea required={showAdvancedOptions} rows={2} className="w-full bg-white border border-slate-200 rounded-xl py-3 px-4 text-xs font-bold resize-none" placeholder="Detailed Address" value={formData.deliveryAddress || ''} onChange={e => setFormData({ ...formData, deliveryAddress: e.target.value })} />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-8">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-[10px] font-bold text-slate-400 mt-1 ml-1">
                                    Base: ₹{((formData.distanceKm || 0) * (formData.ratePerKm || 0)).toLocaleString()} + Extra: ₹{(((formData.fuelCharge || 0) + (formData.serviceCharge || 0) + (formData.repairCharge || 0) + (formData.extraExpense || 0))).toLocaleString()}
                                </p>
                            </div>
                        </div>
